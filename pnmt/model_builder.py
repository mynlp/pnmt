...
import pnmt.modules
from pnmt.encoders import str2enc

from pnmt.decoders import str2dec

from pnmt.modules import Embeddings, CopyGenerator
from pnmt.modules.util_class import Cast
from pnmt.utils.misc import use_gpu
...
def build_task_specific_model(model_opt, fields):
    ...
    if model_opt.model_task == ModelTask.SEQ2SEQ:
        ...
        return pnmt.models.NMTModel(encoder=encoder, decoder=decoder)
    elif model_opt.model_task == ModelTask.LANGUAGE_MODEL:
        src_emb = build_src_emb(model_opt, fields)
        decoder, _ = build_decoder_with_embeddings(
            model_opt, fields, share_embeddings=True, src_emb=src_emb
        )
        return pnmt.models.LanguageModel(decoder=decoder)
    else:
...
def build_base_model(model_opt, fields, gpu, checkpoint=None, gpu_id=None):
    ...
    if not model_opt.copy_attn:
        if model_opt.generator_function == "sparsemax":
            gen_func = pnmt.modules.sparse_activations.LogSparsemax(dim=-1)
        else:
    ...
    if checkpoint is None or model_opt.update_vocab:
        ...
        if hasattr(model, "encoder") and hasattr(model.encoder, "embeddings"):
            ...
            elif model_opt.use_pre_trained_model_for_encoder:
                pass #now the encoder is the pre-trained model so we do not use the word embedding thus initializing the word embedding is not meaningful
            else:
