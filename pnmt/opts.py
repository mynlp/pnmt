...
from pnmt.models.sru import CheckSRU
from pnmt.transforms import AVAILABLE_TRANSFORMS
from pnmt.constants import ModelTask
from pnmt.modules.position_ffn import ACTIVATION_FUNCTIONS
from pnmt.modules.position_ffn import ActivationFunction
...
def model_opts(parser):
    ...
    group.add('--decoder_type', '-decoder_type', type=str, default='rnn',
              ...
              help="Type of decoder layer to use. Non-RNN layers "
                   "are experimental and BERT is not a valid argument for decoder. Additionally, you cannot use bert_decoder. Options are "
                   "[rnn|transformer|cnn|transformer].")
    group.add('--layers', '-layers', type=int, default=-1,
              help='Number of layers in enc/dec. If the encoder is set as BERT, this argument does not have any actual usage')
    group.add('--enc_layers', '-enc_layers', type=int, default=2,
              help='Number of layers in the encoder. If the encoder is set as BERT, this argument does not have any actual usage')
...
def _add_train_general_opts(parser):
    ...
    group.add('--learning_rate_for_pretrained', '-learning_rate_for_pretrained', type=float, default=1e-5,
              help="Starting learning rate for pre_trained model and only used while the use_pre_trained_model_for_encoder is True.")
