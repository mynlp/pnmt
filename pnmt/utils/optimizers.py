...
def build_torch_optimizer(model, opt):
    ...
    use_pre_trained_model = opt.use_pre_trained_model_for_encoder
    ...
    if opt.optim == 'sgd' and not use_pre_trained_model:
        ...
        optimizer_grouped_parameters = [
            {'params': [p for n, p in pre_train_param if ('encoder' in n) & any(nd in n for nd in no_decay)],
             'lr': opt.learning_rate_for_pretrained, 'weight_decay': 0.0},
            {'params': [p for n, p in pre_train_param if ('encoder' in n) & (not any(nd in n for nd in no_decay))],
